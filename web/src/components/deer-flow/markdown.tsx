...
export function Markdown({
  ...
  // Disable heavy animations/rehype for very long content to avoid UI jank
  const isVeryLong = typeof children === "string" && (children?.length ?? 0) > 20000;
  const safeAnimated = animated && !isVeryLong;
  ...
  // Detect whether content likely contains math to decide loading math-related plugins
  ...
  return (
    <div className={cn(className, "prose dark:prose-invert")} style={style}>
      <ReactMarkdown
        remarkPlugins={containsMath ? [remarkGfm, remarkMath] : [remarkGfm]}
        rehypePlugins={rehypePlugins}
        components={components}
        {...props}
      >
        {autoFixMarkdown(
          // Skip KaTeX pre-processing for very long content to reduce CPU
          dropMarkdownQuote(isVeryLong ? (children as string) ?? "" : processKatexInMarkdown(children ?? "")) ?? "",
        )}
      </ReactMarkdown>
