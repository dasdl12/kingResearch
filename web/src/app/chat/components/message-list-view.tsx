  ...
  if (message) {
    if (
      ...
      if (message.agent === "planner") {
        ...
        const isLong = (message.content?.length ?? 0) > 8000;
...
const GREETINGS = ["太棒了", "听起来不错", "看起来很好", "很好", "非常好"];
function PlanCard({
  className,
  message,
  interruptMessage,
                ...
                {plan.steps && (
                  <ul className="my-2 flex list-decimal flex-col gap-4 border-l-[2px] pl-8">
                    {plan.steps.map((step, i) => (
                      <li key={`step-${i}-${step.title ?? ''}-${step.description ?? ''}`} style={{ wordBreak: 'break-all', whiteSpace: 'normal' }}>
